...
def generate_point_matrix():
    if os.path.exists('../ground_truth/{}/Point_dis_matrix.npy'.format(dataset)):
        res = np.load('../ground_truth/{}/Point_dis_matrix.npy'.format(dataset))
    else:
        res = np.load('./ground_truth/{}/Point_dis_matrix.npy'.format(dataset))
    return res
distance_matrix = generate_point_matrix() # When you run this file, you should annotate this sentence
...
def batch_similarity_ground_truth(valiortest = None):
    ...
    if valiortest == 'vali':
        node_list_int = node_list_int[10000:15000]  # based dataset and "validation or test"  (train:validation:test = 1w:4k:1.6w)
        sample_list = node_list_int[:15000]  # m*n matrix distance, m and n can be set by yourself
    elif valiortest == 'test':
        node_list_int = node_list_int[15000:20000]
        sample_list = node_list_int[:5000]
    elif valiortest == 'train':
        node_list_int = node_list_int[:10000]
...
def generate_node_edge_interation():
    node_edge_dict = collections.defaultdict(set)
    if os.path.exists('../data/{}/road/edge_weight.csv'.format(dataset)):
        edge = pd.read_csv('../data/{}/road/edge_weight.csv'.format(dataset))
    else:
        edge = pd.read_csv('./data/{}/road/edge_weight.csv'.format(dataset))
    node_s, node_e = edge.s_node, edge.e_node
...
hot_node_id = hot_node() # When you run this file, you should annotate this sentence
...
if __name__ == '__main__':
    ...
    # x = find_maximal_cliques(roadnetwork)
    ...
    print("raw method similarity time:", time.time() - s1)
