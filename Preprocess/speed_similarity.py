...
def batch_speedlist_ground_truth(valiortest = None):
    time_list_int = np.load(str(config["shuffle_speed_file"]), allow_pickle=True)
    if valiortest == 'vali':
        time_list_int = time_list_int[10000:15000]   # based dataset and "validation or test" (train:validation:test = 1w:4k:1.6w)
        sample_list = time_list_int[:5000]  # m*n matrix distance, m and n can be set by yourself
    elif valiortest == 'test':
        time_list_int = time_list_int[15000:20000]
        sample_list = time_list_int[:5000]  # m*n matrix distance, m and n can be set by yourself
    elif valiortest == 'train':
        time_list_int = time_list_int[:10000]
    ...
    for i in range(len(sample_list)+1):
        if i!=0 and i%50==0:
            pool.apply_async(speedlist_distance, (i, sample_list[i-50:i], time_list_int, valiortest))
    pool.close()
    pool.join()

    return len(sample_list)

def speedlist_distance(k, sample_list = [[]], test_list = [[]], valiortest=None):
    ...
    if not os.path.exists(p):
        os.makedirs(p)
    np.save('../ground_truth/{}/{}/{}_batch/{}_speed_distance_{}.npy'.format(dataset, str(config["distance_type"]), valiortest, str(config["distance_type"]), str(k)), all_dis_list)
...
def find_trajspeedlist():
    longest_traj = 0
    smallest_speed = np.inf
    speed_list_int = np.load(str(config["shuffle_speed_file"]), allow_pickle=True)
    for speed_list in speed_list_int:
        if len(speed_list)>longest_traj:
            longest_traj = len(speed_list)
        for t in speed_list:
            if t<smallest_speed:
                smallest_speed = t
    return longest_traj, smallest_speed

longest_trajtime_len, smallest_trajspeed = find_trajspeedlist()
@numba.jit(nopython=True, fastmath=True)
...
def NetERP_dis(list_a = [], list_b = []):
    ...
    for i in range(lena + 1):
        edit[i][0] = i * smallest_trajspeed
    for i in range(lenb + 1):
        edit[0][i] = i * smallest_trajspeed

    for i in range(1, lena + 1):
        for j in range(1, lenb + 1):
            edit[i][j] = min(edit[i - 1][j] + list_a[i-1] - smallest_trajspeed, edit[i][j - 1] + list_b[j-1] - smallest_trajspeed, edit[i - 1][j - 1] + abs(list_a[i-1] - list_b[j-1]))

    return edit[-1][-1]

def merge_speedlist_ground_truth(sample_len, valiortest):
    res = []
    for i in range(sample_len+1):
        if i!=0 and i%50==0:
            res.append(np.load('../ground_truth/{}/{}/{}_batch/{}_speed_distance_{}.npy'.format(dataset, str(config["distance_type"]), valiortest, str(config["distance_type"]), str(i))))
    res = np.concatenate(res, axis=0)
    np.save('../ground_truth/{}/{}/{}_speed_distance.npy'.format(dataset, str(config["distance_type"]), valiortest), res)

if __name__ == '__main__':

    sample_len = batch_speedlist_ground_truth(valiortest='vali')
    merge_speedlist_ground_truth(sample_len=sample_len, valiortest='vali')
    sample_len = batch_speedlist_ground_truth(valiortest='test')
    merge_speedlist_ground_truth(sample_len=sample_len, valiortest='test')