...
def read_graph(dataset):
    ...
    print("{0} road netowrk has {1} edages.".format(dataset, edge_index.shape[0]))
    print("{0} road netowrk has {1} nodes.".format(dataset, num_node))
...
def save_embeddings(model, num_nodes, dataset, device):
    ...
    return
def prepare_dataset(trajfile, timefile, speedfile, kseg = 5):
    """
    ...
    speed_list = pd.read_csv(speedfile)  
    ...
    for tlist in time_list:
        ...
        for t in tlist:
            if t != '':
                tmp_list.append(int(float(t)))
        time_list_int.append(tmp_list)
    time_list_int = np.array(time_list_int)

    speed_list_int = []  # 1
    for dlist in speed_list:
        tmp_list = []
        dlist = dlist[1:-1].replace('[', '').replace(']', '').replace(' ', ',').replace('\n', ',').split(',')
        for t in dlist:
            if t != '':
                tmp_list.append(int(float(t)))
        speed_list_int.append(tmp_list)
    speed_list_int = np.array(speed_list_int)  # 1
    ...
    speed_list_int = speed_list_int[shuffle_index] 
    ...
    np.save(str(config["shuffle_speed_file"]), speed_list_int)

class Date2vec(nn.Module):
    def __init__(self):
        super(Date2vec, self).__init__()
        self.d2v = Date2VecConvert(model_path="../Models/d2v_model/d2v_98291_17.169918439404636.pth")
...
if __name__ == "__main__":
    ...
    prepare_dataset(trajfile=str(config["traj_file"]), timefile=str(config["time_file"]), speedfile=str(config["speed_file"]), kseg=config["kseg"])
